...
public class GhostScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Find HP text with null check
        GameObject hpObject = GameObject.Find("Canvas/HP");
        if (hpObject != null)
        {
            HP_text = hpObject.GetComponent<Text>();
            if (HP_text != null)
            {
    ...
    void Update()
    {
        STATUS();
        GRAVITY();
        // Respawn() disabled - conflicts with player jump (Space bar)
        // Respawn();
